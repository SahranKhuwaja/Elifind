...
const hire = (id) => {
    ...
    $.post('/Profile/Hire', { id }, (data, status, xhr) => {
        if (data === true && status === 'success') {
            ...
            hiredBy(id, user = false);
            if(userID){
                socket.emit('notification', {myID, userID, notificationText: 'recently hired you in its', notificationAbout: 'company'})
                socket.emit('notification',{myID,userID,notificationText:'Welcome to our',notificationAbout:'company',welcome:true})
...
const unHire = (id) => {
    $.post('/Profile/Unhire', { id }, (data, status, xhr) => {
        ...
        if (data === true && status === 'success') {
            ...
            hiredBy(id, user = false);
            if(userID){
                socket.emit('notification', {myID, userID, notificationText: 'recently expelled you from its', notificationAbout: 'company'})
...
const leave = (id) => {
    ...
    $.post("/Profile/Leave", { id }, (data, status, xhr) => {
        ...
        hiredBy(id, user = true);
        socket.emit('notification', { myID, userID:companyID, notificationText: 'recently left your', notificationAbout: 'company', mediaName: companyName })
