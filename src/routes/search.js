...
router.get('/Profile/View/:id/Portfolios',auth,async(req,res)=>{

  if(req.params.id === (req.user._id).toString()){
  
    return res.redirect('/Profile/Portfolios');
  }
  ...
  res.render('portfolios',{data,image,Pimage,loggedInUser,created:moment(userData.createdAt).fromNow(),updated:moment(userData.updatedAt).fromNow(),age:age });
